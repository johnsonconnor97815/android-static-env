...
LOCK = setup.SKILL / 'assets/mcp.lock.json'
...
class MCPEnvironment(setup.Environment):
    ...
    def launch(self, name, command, extra=None):
        ...
        target.chmod(0o755)
        return {'command': str(target), 'args': [], 'cwd': str(cwd)}
    ...
    def install_server(self, name):
        ...
        if name == 'ghidra':
            ...
            shutil.copyfile(setup.SKILL / 'scripts/ghidra_mcp_entry.py', entry)
            return self.launch(name, [self.root / 'venvs/mcp-ghidra/bin/python', entry, '--model-cache', model,
                '--transport', 'stdio', '--project-path', project,
                '--project-name', 'android-analysis'], {'GHIDRA_INSTALL_DIR': str(ghidra)})
        # Keep Semgrep on local, explicit-rule operations in this static-analysis profile.
        return self.launch(name, [self.root / 'bin/semgrep', 'mcp', '-t', 'stdio'], {
    ...
    def templates(self, specs):
        ...
        if 'android-jadx' in specs:
            setup.write_json(folder / 'jadx-optional.json', {'mcpServers': {'android-jadx': {
                'command': specs['android-jadx']['command'], 'args': []}}})
...
def main():
    ...
    try:
        ...
        if args.action == 'configure':
            ...
            specs = {f'android-{n}': all_specs[f'android-{n}'] for n in env.names}
